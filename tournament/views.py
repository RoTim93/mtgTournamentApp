...
def tournament_players(request, id):
    ...
    players.sort(key=lambda player: player.match_points, reverse=True)  # Sort players by match points
...
def update_results(request, tournament_id):
    if request.method == 'POST':
            ...
            for i, pair in enumerate(pairings.filter(round=round_number), start=1):
                ...
                if player1_id:
                    ...
                    if pair.result and not pair.was_bye:
                        if pair.player1_score is not None and pair.player2_score is not None:
                            if pair.player1_score > pair.player2_score:
                                ...
                                if player2:
                                    player2.losses -= 1
                            elif pair.player2_score > pair.player1_score and player2:
                    ...
                    if player2_id == 'Bye':
                        ...
                        print(f"Skipping pair {i} due to empty scores")
                        continue

                    else:
                        new_score1 = int(score1) if score1 else 0
                        new_score2 = int(score2) if score2 else 0

                        print(f"Processing pair {i} in round {round_number}")
                        print(f"Player 1 ID: {player1_id}, Player 2 ID: {player2_id}, Score1: {score1}, Score2: {score2}")

                        if new_score1 == 0 and new_score2 == 0:
                            pair.player1_score = 0
                            pair.player2_score = 0
                            pair.result = '0-0'
        ...
        for player in players:
            print(f"Player {player.name}: Match points: {player.match_points}, Wins: {player.wins}, Draws: {player.draws}, Had bye: {player.had_bye}")
